# Bikash Giri — Data Analyst & Java Developer

[![LinkedIn](https://img.shields.io/badge/LinkedIn-bikash--kumar--giri-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/bikash-kumar-giri-60ab32288)
![Location](https://img.shields.io/badge/📍-Oakville%2C%20Ontario%2C%20Canada-green?style=flat)

---

Computer Systems Technology student at **Sheridan College** (graduating April 2027), based in **Oakville, Ontario, Canada**. I'm building hands-on experience in data analysis and backend Java development — working toward a career as a **Data Analyst or Junior Developer** in Canada.

I enjoy turning raw data into clear insights and building well-structured Java applications. Currently deepening my skills in Python-based data workflows and completing the Google Data Analytics Certificate.

---

## Tech Stack

**Data & Analytics**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)

**Backend Development**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat&logo=springboot&logoColor=white)
![Thymeleaf](https://img.shields.io/badge/Thymeleaf-005F0F?style=flat&logo=thymeleaf&logoColor=white)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=flat&logo=apachemaven&logoColor=white)

---

## Certifications

| Certificate | Issuer | Status |
|---|---|---|
| Python for Everybody | University of Michigan (Coursera) | ✅ Complete |
| Data Science Math Skills | Duke University (Coursera) | ✅ Complete |
| Foundations: Data, Data Everywhere | Google (Coursera) | ✅ Complete |
| Prepare Data for Exploration | Google (Coursera) | ✅ Complete |
| Google Data Analytics Certificate | Google (Coursera) | 🔄 In Progress |

---

## Projects

| Project | Description | Status |
|---|---|---|
| [data-analysis-python](https://github.com/giri-bikash/data-analysis-python) | Exploratory data analysis using Python, Pandas, and Matplotlib | 🔄 In Progress |
| [card-game-java](https://github.com/giri-bikash/card-game-java) | Java card game demonstrating OOP principles (classes, encapsulation, game logic) | ✅ Complete |
| [Spring Boot Sheridan](https://github.com/giri-bikash/bikash-giri) | Spring Boot MVC web application built at Sheridan College | ✅ Complete |

---

## GitHub Stats

![Bikash's GitHub Stats](https://github-readme-stats.vercel.app/api?username=giri-bikash&show_icons=true&theme=dark&hide_border=true)

---

*Oakville, Ontario, Canada · Open to Data Analyst & Developer roles*
